import { VOLATILITY_DURATIONS } from "@/data/indexDB/constants/VOLATILITY_DURATIONS"
...
export default function BalancePage({ name = "StatusesBalancePagePage", ...rest }: PropsWithChildren<ComponentProps>) {
  ...
  const [showProfit] = useState<boolean>(true)
  ...
  useEffect(
    () => {
      if (symbol == null) {
        const selectedSymbol = activeMarkets?.[0]?.symbol

        setSymbol(selectedSymbol)
      }

      async function run(symbol: string, notional: number) {
        const analysis = await generateAnalysis(symbol, notional, duration)
    ...
    [timer, symbol, notional, duration]
  )
  ...
  return (
    <div {...rest} data-component={name}>
      ...
      <div className="flex flex-row gap-2 p-2 items-center">
        ...
        {VOLATILITY_DURATIONS?.map((activeDuration) => {
          const displayClassName = activeDuration === duration ? "btn btn-primary" : "btn btn-outline btn-primary"

          const handleSelectDuration = (selection: number) => () => {
            setDuration(selection)
          }

          return (
            <button className={displayClassName} key={activeDuration} onClick={handleSelectDuration(activeDuration)}>
              {activeDuration}
            </button>
          )
        })}
...
const TradeColumn = ({ source, showDelta = false }: { symbol: string | undefined; source: any; showDelta?: boolean }) => {
...
const ProfitColumn = ({
  ...
  if (showBuyCall != null) {
    const buyCallIndex = source.buyCall.contracts?.findIndex((contract: any) => contract.delta === showBuyCall)

    if (buyCallIndex > -1) {
      contractCost += source.buyCall.contracts[buyCallIndex].contractCost
    }
  }
  ...
  } else if (displayMaxMarginCall != null) {
    displayMargin = displayMaxMarginCall
  }
...
const OptionRow = ({
  ...
  if (direction === OptionDirection.BuyCall || direction === OptionDirection.BuyPut) {
    if (profit <= cost * -1) {
      profitClassName += " text-error"
    } else if (profit > 0) {
      profitClassName += " text-success"
  ...
  } else if (direction === OptionDirection.SellCall || direction === OptionDirection.SellPut) {
    if (profit < 0) {
      profitClassName += " text-error"
    } else if (profit >= cost) {
      profitClassName += " text-success"
...
const OptionColumn = ({
  ...
  return (
    <div className="w-24 flex flex-col justify-start items-center gap-2 p-2">
      ...
      <div className="divider"></div>
      {source.outcomes?.map((item: any, i: number) => (
        <OptionRow key={i} item={item} cost={contractCost} contractDelta={contractDelta} direction={direction} onClick={onClick} />
      ))}
    </div>
  )
}
