...
<<<<<<< HEAD:src/data/indexDB/controllers/ratesLoadFor.ts
=======
export async function controller(db: PriceSimulatorDexie, key: string) {
  ...
  const response = await fetch(`/rates/` + path, {})
...
>>>>>>> 371de67 (pre 4 tailwind):src/data/indexDB/controllers/loadRateForKey.ts
